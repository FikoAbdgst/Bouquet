...
<body class="bg-[#FFFDFC] min-h-screen flex flex-col font-sans antialiased">
    ...
    @if (!($hideNav ?? false))
        <nav class="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-5xl">
            <div class="relative">
                {{-- Pill Container --}}
                <div id="navbar-pill"
                    class="bg-white/70 backdrop-blur-md border border-[#E7E4DC]/60 rounded-full px-3 py-1.5 transition-[background-color,border-color,box-shadow] duration-300">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="hidden md:flex items-center space-x-1">
                            <a href="{{ route('home') }}"
                                class="text-[#6E8577] hover:text-[#D37897] font-medium text-sm tracking-wide transition px-4 py-2 rounded-full hover:bg-[#F1F0EA]">
                                Beranda
                            </a>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <a href="{{ route('customer.cart') }}"
                                class="relative text-[#6E8577] hover:text-[#D37897] transition p-2 rounded-full hover:bg-[#F1F0EA]">
                                ...
                                @auth
                                    ...
                                    <div class="relative">
                                        ...
                                        <div id="user-dropdown"
                                            class="hidden absolute right-0 mt-2 w-52 bg-white rounded-2xl border border-[#E7E4DC] py-1.5 z-50 overflow-hidden">
                                            <div class="px-4 py-3 border-b border-[#E7E4DC]/50">
                                                <p class="text-sm font-semibold text-[#33413A]">{{ Auth::user()->name }}
                                                </p>
                                                <p class="text-xs text-[#6E8577] mt-0.5">{{ Auth::user()->email }}</p>
                                            </div>
                                            @if (Auth::user()->isCustomer())
                                                <a href="{{ route('customer.orders.index') }}"
                                                    class="block px-4 py-2.5 text-sm text-[#6E8577] hover:bg-[#F1F0EA] hover:text-[#D37897] transition">Pesanan
                                                    Saya</a>
                                            @endif
                                            <form action="{{ route('logout') }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="w-full text-left px-4 py-2.5 text-sm text-[#6E8577] hover:bg-[#F1F0EA] hover:text-[#D37897] transition">Logout</button>
                                            </form>
                                        </div>
                                    </div>
                                @else
                                    <a href="{{ route('login') }}"
                                        class="border border-[#D37897] text-[#D37897] hover:bg-[#D37897] hover:text-white font-medium text-sm tracking-wide transition px-5 py-2 rounded-full">Login</a>
                                    <a href="{{ route('register') }}"
                                        class="bg-[#D37897] text-white px-5 py-2 rounded-full hover:bg-[#C06A85] font-medium transition text-sm tracking-wide">Daftar</a>
                                @endauth
                            </div>

                            {{-- Mobile Hamburger --}}
                            <button id="mobile-menu-btn"
                                class="block md:hidden text-[#6E8577] hover:text-[#D37897] focus:outline-none p-2 rounded-full hover:bg-[#F1F0EA]">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path id="menu-icon-open" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                                    <path id="menu-icon-close" class="hidden" stroke-linecap="round"
                ...
                {{-- Mobile Menu --}}
                <div id="mobile-menu"
                    class="hidden md:hidden absolute top-full left-0 w-full mt-2 p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-[#E7E4DC]">
                    <div class="space-y-1">
                        <a href="{{ route('home') }}"
                            class="block py-2.5 px-3 text-[#6E8577] hover:text-[#D37897] hover:bg-[#F1F0EA] rounded-xl font-medium transition">Beranda</a>
                        <a href="{{ route('customer.catalog') }}"
                            class="block py-2.5 px-3 text-[#6E8577] hover:text-[#D37897] hover:bg-[#F1F0EA] rounded-xl font-medium transition">Katalog
                            Bunga</a>
                        <a href="{{ route('customer.cart') }}"
                            class="flex items-center justify-between py-2.5 px-3 text-[#6E8577] hover:text-[#D37897] hover:bg-[#F1F0EA] rounded-xl font-medium transition">
                            <span>Keranjang</span>
                            <span
                        ...
                        @auth
                            ...
                            <div class="border-t border-[#E7E4DC] pt-3 mt-3">
                                ...
                                <form action="{{ route('logout') }}" method="POST" class="mt-2">
                                    @csrf
                                    <button type="submit"
                                        class="w-full text-left px-3 py-2.5 text-sm text-[#6E8577] hover:bg-[#F1F0EA] hover:text-[#D37897] rounded-xl font-medium transition">Logout</button>
                                </form>
                            </div>
                        @else
                            <div class="border-t border-[#E7E4DC] pt-3 mt-3 space-y-1">
                                <a href="{{ route('login') }}"
                                    class="block text-center border border-[#D37897] text-[#D37897] hover:bg-[#D37897] hover:text-white py-2.5 rounded-xl font-medium tracking-wide transition">Login</a>
                                <a href="{{ route('register') }}"
                                    class="block text-center bg-[#D37897] text-white px-4 py-2.5 rounded-xl hover:bg-[#C06A85] font-medium tracking-wide transition">Daftar</a>
                            </div>
                        @endauth
                    </div>
                </div>
    ...
    {{-- Main Content --}}
    <main
        class="flex-1 max-w-7xl mx-auto w-full {{ $hideNav ?? false ? 'pt-8' : 'pt-20' }} pb-8 px-4 sm:px-6 lg:px-8">
        @yield('content')
    </main>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var navbarPill = document.getElementById('navbar-pill');
            if (navbarPill) {
                var updateNavbar = function() {
                    if (window.scrollY > 24) {
                        navbarPill.classList.add('bg-[#F1F0EA]/90', 'shadow-lg', 'shadow-[#D37897]/10');
                        navbarPill.classList.remove('bg-white/70');
                    } else {
                        navbarPill.classList.remove('bg-[#F1F0EA]/90', 'shadow-lg', 'shadow-[#D37897]/10');
                        navbarPill.classList.add('bg-white/70');
                    }
                };
