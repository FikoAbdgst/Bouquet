...
**Global toasts:** `partials/toasts.blade.php` is included by both layouts (`app` + `admin`). It renders one toast per flash (`session('success'|'error'|'warning'|'info')`, string or array values) with a floral/pastel design, inline `<style>` keyframes (`toast-in`/`toast-out`/`toast-progress`), and inline JS (auto-dismiss, close button, staggered entrance). The old static flash banners in `layouts/*.blade.php` and `order-create.blade.php` were removed — don't re-add them or messages double-display. Validation errors stay inline via `@error`; only server-side flash messages become toasts. The partial also exposes a **client-side API** `window.BuketToast.show(type, message)` (types: `success`/`error`/`warning`/`info`) that creates a toast on the fly — used by the add-to-cart handlers in `product-detail.blade.php` and `catalog.blade.php` (quick add), since those are pure localStorage actions with no server flash. Prefer `BuketToast.show()` for client-side feedback instead of ad-hoc inline elements.
